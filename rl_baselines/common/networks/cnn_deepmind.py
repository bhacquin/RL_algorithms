...
class CNNDeepmind_Threehead(nn.Module):

    def __init__(self, observation_space, n_outputs_1, n_outputs_2, n_outputs_3, width=512):
        # CNN architechture of DeepMind described in https://storage.googleapis.com/deepmind-media/dqn/DQNNaturePaper.pdf :
        ...
        self.output_3 = nn.Sequential(nn.Linear(64 * 7 * 7, width),
                            nn.ReLU(),
        ...
        self.output_1.apply(lambda x: init_weights(x, np.sqrt(2)))
        self.output_2.apply(lambda x: init_weights(x, np.sqrt(2)))
        self.output_3.apply(lambda x: init_weights(x, np.sqrt(2)))
    ...
    def forward(self, obs):
        ...
        return self.output_1(obs), self.output_2(obs), self.output_3(obs)

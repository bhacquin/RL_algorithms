...
class SubProcessEnv(Process):
    ...
    *Note*: If the environment steps into terminal state (``done == True``) it immediately resets the envoronment
    ...
    def __init__(self, env_id, master, slave, wrapper = None):
    ...
    # def make_carracing(self,env_id, color_range=0):
    #
    #     env = gym.make(env_id)
    ...
    def run(self):
        ...
        # env = self.make_carracing(self.env_id)
        steps = 0
        collected_reward = 0
        while True:
...
class MultiEnv(Env):
    ...
    *Note*: Since this environment runs multiple ones underneath it never finishes. If one of the environments returns
    ...
    def __init__(self, env_id, n_envs: int, wrapper = None):
        """
        ...
        self.processes = [SubProcessEnv(env_id, *Pipe(), wrapper=wrapper) for _ in range(self.n_envs)]
        self._start()
